...
// --- MODIFIED SPLASH SCREEN LOGIC (No Image) ---
window.onload = function() {
    ...
    // 1. Immediately show the DERMATO text splash screen
    mainSplashScreen.classList.remove('hidden');
    
    // 2. Start the DERMATO animation (3s CSS animation)
    splashText.classList.add('animate');

    // 3. Wait for animation to finish, then show app
    setTimeout(() => {
            // A. Start the 0.5s fade-out transition
            mainSplashScreen.classList.add('hidden');

            // B. Wait for fade-out, then start main app
